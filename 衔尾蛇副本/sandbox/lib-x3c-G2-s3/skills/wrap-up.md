# wrap-up · 任务收尾（library-bootstrap v3.8.2 渲染产物）
...
3. **衰减与对账**：`engine wrapup --lib .` ——四维情绪 ×0.7、Merkle 根重算抄录人侧。
4. **反思检查**：wrapup 输出「反思触发」时（importance_accum ≥ 50），执行反思合成（Generative Agents 式：跨任务规律 2~3 条，无据不写），并重置累计器。
