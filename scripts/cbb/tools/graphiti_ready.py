...
用法：
  py -X utf8 graphiti_ready.py [--store <本项目>-本体库] [--json]
  py -X utf8 graphiti_ready.py --sentinel --store <本项目>-本体库
"""
...
def trigger_sentinel(store_root: Path | str, logs_dir: Path | str | None = None,
    ...
    import 路径惯例 as 惯
    logs = Path(logs_dir) if logs_dir else 惯.workspace_of(store) / "logs"
