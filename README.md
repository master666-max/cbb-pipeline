...
> **v2.0（2026-09-22）**：不再只服务小说——任意可切分的长文本语料都行（换语料只换"切分规则＋资料分类＋词表"三样参数）；新增审查体制（五层证据门＋按执行批次分层抽样＋双复查估计"还有多少错没被发现"）；项目参数里可自定义资料类别与受控词表。小说实例（517 章实测）已归档为参照样例。
...
| 属性 | 表现 |
|---|---|
| **成本可为 0** | 整条流水线能在免费通道上跑完全书（实测项目即如此） |
