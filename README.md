...
- **关系图不当仓库，但它是必须有的"参考图"——不是可有可无的装饰。** 资料库本体永远是纯文本文件（每条结论能逐字翻回原文核对、能 diff、能断点续跑）；关系图是照着文件另画的关系网，专门干一件事：**跨章关系巡检**——谁引用了不存在的人、死掉的人又出场、隔了几百章的身份对不对得上。这些在文件里要全库翻找，在图上一条查询就出来。规矩是两头都要：**入库不依赖它**（它坏了先记账，照常干活），**但最终大检查之前必须把账还清**（导出债务清零才许做终审）。
