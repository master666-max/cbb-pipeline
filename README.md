...
- **关系图是视图，不是仓库。** 资料库本体永远是纯文本文件——每条结论带原文出处、能逐字翻回去核对、能 diff、能断点续跑。图数据库擅长的是"顺着关系网爬"，不擅长"证明这条结论没抄错"，所以它只当**可选的第二视图**：随时能从文本重建，坏了也不影响主流程。
- **Docker 只有这一处需要，而且默认不开。** 装图数据库（Neo4j）才要用 Docker；不用就完全不装，主流程一行不改。语义查重（本地嵌入服务）同样是可选增强。
