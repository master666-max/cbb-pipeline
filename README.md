...
| 属性 | 表现 |
|---|---|
| **成本透明（不吹零成本）** | 单章约 **3 万–4.5 万 token**（实测折算：单章输入 49KB＋输出 47KB；中文约 1 token ≈ 2.5–4 字节）。已跑 230 章 ≈ **700 万–1000 万**，全书 517 章预计 **1600 万–2400 万 token**。可以跑在免费额度上（现金 0），但 token、额度、人工与本地算力都是真实消耗——按主流 API 价位折算约 **¥40–160** |
