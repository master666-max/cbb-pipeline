...
## 阶段9 M 链审计与俯瞰——已完成（2026-09-10，v2 规格）

- 规则预注册：`tools/phase9-guide.md`；计算 `tools/phase9_status.txt`。
- 产出登记（wc -l + sha256[:16]）：

| 文件 | 行数 | sha256[:16] |
|---|---|---|
| 09-M-STATUS.md | 70 | 82c4dc7788c2bc72 |
| delta-baseline.csv | 20（19+表头） | 4ddb89b7adc9a3bd |
| data/phase9-summary.txt | 38 | d778b1af59849600 |

- 关键数字【机械事实】：新机制按出生宇宙 SIM 35/DS 4/REAL 4/PART2 2；红队扑空率 75.0%（12 出边 refutes 3）；三件套齐备率 1/45（MECH-23 唯一全齐，registry 语料口径）；DATA-DRIVEN 13/26=50.0%；边数口径区间 [118(实证),173(全部)]。
- 关键分类决策（≤5）：
  1. **第三处名实差异登记**：规格 M0-M6/推演螺旋全工作区 grep 0 命中——按外部框架逐项对标（"规格Mk"防与 DS 线机制发现 M1-M3 混用），判定 M0/M1/M3/M4 达成、M2 部分、M5 未启动（两轴:47 待办锚点）、M6 部分（沙盒态）。
  2. 螺旋位置判定=第一圈出口/第二圈入口（M1 首图=本任务；delta 曲线自第二圈起）。
  3. M0 退出三指标：机制级稳定（12 证伪全 SIM 期、post-SIM=0）vs 断言级翻土（52 refuted 持续分布于后期宇宙）并陈——有条件倾向退出。
  4. 三件套 1/45 读法定性为**登记层欠账**（非机制不成熟），第二圈回填 registry=阶段10 c 候选。
  5. 旋转门预警：DATA-DRIVEN 50%+机制出生率衰减 → 换道闸必要性中高，LIT-REPLICATE 仅 27% 偏低。
- 边界情况：红队族清单冻结为 {VERIF-*, REVIEW-*}（PART2-RND-12 红队行无出边不计入）；L8 21/21 锚点用 task-006:19（首跑 19/21→修正）+_index.md:74 双引。
- 未解决问题：M0 正式退出留第二圈 delta 曲线确认；MECH-45 部分翻案边界未清（机制清单 unknown 族）。
- 自检三问：见 09-M-STATUS.md §九。

## 阶段10 未来实验分支树设计——已完成（2026-09-10，v2 规格）

- 规则预注册：`tools/phase10-guide.md`（四源配额+七字段纪律+排序规则）。
- 产出登记：`10-branch-proposals.md` 144 行 sha256[:16] `90be9447dc972708`；`tools/phase10-guide.md` 17 行 `1c4f31de4e601139`。
- 关键数字：提案 13 条（a×4/b×3/c×3/d×3）+ 负提案 1 条（P-D4 触发器在位未触发，不立项）；**建议下一圈首批=P-A1 总法则第七验 / P-D1 锚定集独立性谱 / P-B1 复合灾难三条件 / P-C3 三件套回填**；先验概率全部带来源标签（历史频率 3/类比外推 4/均匀默认 5/行动型 1）。
- 关键分类决策（≤5）：①排序按增益/成本定性三档而非伪精确分数；②P-C3 为行动型提案（先验 1.0，"假设"=登记层可达性）；③P-B1 复活与终裁双出路判据都预注册（第三振→定理）；④负提案如实登记不硬凑配额；⑤P-D2 把 EXTERNAL-CLAIM 数字转为"本地保守版"可测陈述（对照不采信）。
- 边界情况：P-A4 开辟型（不翻转旧分支）单列低档——信息价值性质不同，不与翻转型混排。
- 未解决问题：P-A2 特征集设计依赖 E14c 失败模式分析——首圈执行时先做失败模式复盘再定特征。
- 自检三问：见 10-branch-proposals.md 末节。

## 阶段11 终报 + 阶段11.5 红队交接包——已完成（2026-09-10，v2 规格）——**主任务结束**

- 产出登记（wc -l + sha256[:16]）：

| 文件 | 行数 | sha256[:16] |
|---|---|---|
| 99-MAP.md | 82 | 84441bde88b9187c |
| archive-plan.md | 40 | 22e713df9a710c00 |
| red-team-input-manifest.csv | 349（348+表头） | e274ffd8872a2442 |
| data/phase11-stats.txt | 24 | 157000998d39b5ef |

- 99-MAP 要点：一页俯瞰 19 指标全部 `tools/phase11_stats.txt` 机械产出（命令入档零手写）；三层置信度分档；增节《选择方差与两级质量门》（两级门定义+修订史+方法论-01+B 计划预注册+效力边界声明"仅证规则可机械导出不证读者自然一致"）；三处名实差异汇总；研究员自述带显式隔离标记（永不入 csv）。
- archive-plan：只出方案不执行——账本/报告全 CANONICAL；SUPERSEDED 六处（原单阈值门等，均原地标注）；attic 候选 6（4 表片段+2 阶段4 明细建议保留）需人工批准。
- 红队交接包 348 行：已抽 claim_id 321（红队独立抽检必须避开）+ 统计命令 11 + 账本/报告终态快照 16（manifest 自身哈希以 git commit 为准）。
- 边界情况：AGENTS.md 的 wrap-up/reflect 技能本轮**不调用**——R1（原工作区只读）与主工作区冻结公告（knowledge/_index.md:3）禁止向 knowledge/ 写入；任务收口以本 progress.md+99-MAP 为准。
- 任务终态：阶段 0/0.5/1/2/3/4/HALT/裁决/5/6/7/8/9/10/11/11.5 全部完成；共 9 个 commit（首轮 8+续跑 8）；下一圈入口=10-branch-proposals.md 首批四提案+B 计划（升格操作定义重提）+ delta-baseline.csv 机械 diff。

## 追加产物登记：98-冷启动考古总报告（2026-09-11，用户索取）

- **前置核验**：发现主任务结束（1247786）后盘上已有两个后继事件——①红队审计执行（独立会话，毒 0/伤 7/痒 5，不触发 DOWNGRADE-TO-DRAFT，零点资格维持；其间方法论-02 沙盒穿透事故+还原）；②用户裁决勘误五步（commit `5476d70`：audit-log 还原 322 行、incident-log 26→27 行、delta-baseline +1 修正行=21、99-MAP +终审结论块=92 行、errata-log.md 首建 73 行）。本会话逐一读取核实后才开始撰写——总报告覆盖至该真实终态。
- **新产物**：`98-冷启动考古总报告.md`（面向阅读的叙事总报告：缘起/三幕时间线/工作区画像/账本地图/核心发现十条/质量体系/事故修复史八起/遗留与下一圈/复现命令；一切数字转引并带时点，与 99-MAP 权威终报分工）。红队报告本体=仓库根 untracked `red-team-report.csv`+`red-team-output/`（未入 git；commit 后 git status 核实并修正报告原表述"红队会话侧留档"）。
- 99-MAP/archive-plan 本体未再改动（防 manifest 快照进一步漂移；98 的登记以本块为准）。
- 自检三问：报告内全部数字转引自已登记产物且点值带区间/时点；红队环节为盘上事实（errata-log+git show 为据）非推定；主观附注置隔离标记内。
