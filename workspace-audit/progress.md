...
| 文件 | 行数 | 备注 |
|---|---|---|
| gate-revision.md | 79 | 修订记录本体（原定义 superseded 的声明处） |
| incident-log.csv | 2 | **首次建立**（表头+方法论-01 行） |
| alias_map.csv | 52+1 | 追加 方法论-01 行（只增不改） |
| HALT.md | 96 | 追加 §六 裁决解除（§一~§五 原地保留） |
| claim_ledger.csv | 3,941 | +14 条 方法论-01（现行 3,938） |
