"""bootstrap_v3.9.py — library-bootstrap · 超绝统合版单文件安装包
====================================================================
公理：
...
BASELINE_BLACKLIST = ["run_tests", "lint_library", "lint_knowledge", "l8_guard", "anchor",
                      "MUTATION-SOP", "bootstrap_v3.9.py", ".library-manifest.json", "T-REGISTRY"]
...
TUNABLES = {
 ...
 "law_w_content":   {"value": 1.0, "source": "v3.9/W-1：v3沿革值（X2a无变更收益，证据中）", "owner": "engine retrieve"},
 "law_w_imp":       {"value": 1.0, "source": "v3.9/W-1：X2a合成终态多为0、X5真库1.0无害——维持待复测（证据弱）", "owner": "engine retrieve"},
 "law_w_age":       {"value": 0.1, "source": "v3.9/W-1：合成世界病灶实证、真实库未触发（定时炸弹）——可配置化", "owner": "engine retrieve"},
 "law_filter_zero": {"value": 1,   "source": "v3.9/W-1：24种子+真实库20/20双世界复现（证据强），默认开启", "owner": "engine retrieve"},
}
...
LEVELS = {  # 模块名以 absorb 盘点为准，此处为构成语义+已知名；absorb 回填 mapping
 ...
 "L9": {"name":"内省仿生:BCA","adds":["l9:emotion","l9:gwt","l9:sleepgate","l9:instrumentation",
        ...
        "gate":"BCA四公理+验收三件(消融/干预/谄媚)——确认后交付 constitution-bca(append-axioms/ABLATIONS)，安装前可在 bodies 预览,人类确认"},
}
...
DEPS = {"m-evolve": ["m-schema3"], "m-sleep": ["m-reflect"], "l9:emotion": ["l8:governor"],
        ...
        "l9:dmn": ["l9:instrumentation"], "l9:goalstack": ["l9:instrumentation"]}
...
def _deliver(b: dict, mods: list, packages: list, level: str, target: Path):
    ...
    for slot, items in b.items():
        ...
        for key, meta in sorted(items.items()):
            ...
            if "__pycache__" in rel.split("/") or rel.endswith((".pyc", ".pyo")): continue  # v3.8.2/L9-DELIVER：缓存/二进制不交付
            dest = target / rel
...
def cmd_install(level: str, packages: list, target: Path, yes: bool, redeliver: bool = False):
    ...
    state = {"package": "bootstrap_v3.9.py", "package_sha256": self_hash(), "version": VERSION,
