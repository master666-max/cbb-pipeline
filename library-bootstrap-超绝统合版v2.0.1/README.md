# library-bootstrap · 超绝统合版（v2.0.1 · 126 文件）
...
- **v2.0.1（2026-09-05，L9 全档实装后修补）**：
  - 补缺失模板：`experimental-l9/gwt/template/PENDING.md`、`experimental-l9/constitution-bca/template/ABLATIONS.md`（此前 install.md 引用但包内无文件，agent 需手工补）。
  - m-core 补齐模板源：`template/archive/_INDEX.md`、`template/archive/_cold/.keep`、`template/workspace/.keep`，并修正 install.md 文件清单映射（此前只写目标路径、无源模板）。
  - broadcast 说明对齐实际行为：PENDING 队列当前只写不读（install.md 与脚本 docstring 已同步注明"回读属机器变异候选，走 MUTATION-SOP"）。
  - vector-search 脚本加空库守卫：0 条目时 `--build` 优雅退出（原版空库抛 `empty vocabulary` ValueError）。
