...
| 5 | ~~推理优先：投判卷/双独立复查/SPRT~~ **【已挂起 2026-09-25，见 §十一】** | §6.2 |
...
## 十一、挂起登记：推理能力研讨【挂起 2026-09-25】

- **裁定**：用户令「把关于推理能力的研讨挂起」（2026-09-25）。
- **挂起对象**："瓶颈在推理端 → 资源投向判卷/双独立复查/推理端质量"这条**研讨与立项方向**（本报告 §6.2、§7.1、§八#5）。
- **不在挂起范围**：已在案的判卷机制（双独立复查、Chapman 估计、SPRT、G5 结论门）**照常运行**，本裁定不冻结任何既有仪器；挂起的只是"把推理端作为下一投入方向"的后续研讨与工单化。
- **重开条件**（任一满足即可提请重开）：
  1. 任何检索面增强（主题词路、社区摘要、图扩展调参）立项时——须先答"这笔钱投检索还是投推理"；
  2. 终审/判卷出现系统性误判的实证（抽样显示结论门漏判率超预注册阈值）；
  3. 用户显式重开。
- **登记位置**：本报告 §十一 + 决策账 ruling 同日条目。
