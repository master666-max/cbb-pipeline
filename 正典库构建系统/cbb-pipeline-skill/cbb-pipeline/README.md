...
- **关系图不当仓库，但它是贯穿全流程的"参考图"——不是可有可无的装饰。** 资料库本体永远是纯文本文件（每条结论能逐字翻回原文核对、能 diff、能断点续跑）；关系图是照着文件另画的关系网，干一件事：**跨章关系巡检**——谁引用了不存在的人、死掉的人又出场、隔了几百章的身份对不对得上。这些在文件里要全库翻找，在图上一条查询就出来。而且它不是最后才拿出来用的：**每批都在跟着更新**——日常巡检走它，抽取前发给 AI 的"人物小抄"也从它这里查关系，最后的大检查还要拿它跟文件对着清点。两头规矩：**入库不依赖它**（它坏了先记账，照常干活），**但最终大检查之前必须把账还清**（欠账清零才许做终审）。
- **Docker 只有这一处需要。** 装这张参考图（Neo4j）才要用 Docker，不用就不装；一旦启用，按纪律每批收口都要同步更新它、更新不上就记账——欠账没还清，不许进最终大检查。
