...
- U-B07 cbb-store 双轨+约束 | 状态=done(2026-09-15) | 判据=双轨分流+UNIQUE防重+漂移钩子+cbb-merge并入单测绿 | 证据=双轨合并(一致重复→confidence上调max+2.0+证据并集经supersede出新版本；矛盾→quarantine contradiction_pending+契约合规Verdict不静默合并；无既存→ON CREATE)；UNIQUE约束族(别名复合PK/关系三元组唯一/出场唯一)；verified_against漂移钩子(drift_check+stale_records重验门)；ON CREATE/ON MATCH=admit_or_merge；时序回放(state_changes顺序覆盖任意章快照)；安全围栏(根围栏四层逃逸拦截+symlink拒写实测+命令白名单思想)；cbb-merge并入(multiversion Keep/Range/Flag/Pick+冲突表)；保留三态+supersedes；18单测绿(基线11,R4真缺陷1处被测试抓住当场修复)；GPL仓webnovel-writer按设计重写零源码引用；十轮自审=cbb/自审日志/U-B07.md | 幂等=skip-if-exists+旁车终态守卫
- U-B08 集成冒烟 | 状态=done(2026-09-15) | 判据=离线版全绿双跑EXIT0；真管道版done或blocked留痕 | 证据=离线版74/74断言PASS真实EXIT0(coordinate→anchor双时间轴→extract四面防御→gate1三域Issue→quarantine三子类urgency→store双轨六段全链,同目录双跑:摘要全等+文件集不变+库文件逐字节不变)；真管道版=blocked留痕(探活实测:LM Studio8080=True/Neo4j7687=False/凭证双True→留阶段三,cbb/smoke/真管道探活-2026-09-15.md含就绪条件)；集成双跑暴露store两真缺陷当场修(find_by_identity取活版本+证据子集幂等守卫,store 18→19测)；全库单测总数161≥73；十轮自审=cbb/自审日志/U-B08.md | 幂等=缓存+skip-if-exists
- U-B09 终报交接 | 状态=done(2026-09-15) | 判据=交接文书+实验版对比表(73 vs 新总数) | 证据=交接文书落盘=cbb/交接文书-本体构筑.md(七节:判据逐项对照5/5/单位终态含十轮出口门列/git摘要10笔/待确认11条/经验候选3条带实据/终跑161+对比表73vs161+冒烟20vs74/行为差异6条)；十轮自审=cbb/自审日志/U-B09.md(R9/R10零发现) | 幂等=快照型

前置事实（2026-09-15 审核线核证）：工单 v1.0 在位（唯一指令源）；实验版 FROZEN（只读，测试须 -B）；阶段一验收 17/17（task-047）；用户三裁决已录入工单 §0。

阻塞登记：真管道版 graphiti 实入库=blocked（U-B08 探活实测：LM Studio 8080 在线、Neo4j 7687 未监听——留阶段三，就绪条件见 cbb/smoke/真管道探活-2026-09-15.md；按工单不阻塞离线收口）。
勘误日志：（无）
